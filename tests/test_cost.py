...
def test_record_computes_usd_from_pricing_table():
    tracker = CostTracker()
    tracker.record("openai/gpt-5.6-luna", input_tokens=1_000_000, output_tokens=1_000_000)
    assert tracker.total_usd == 1.40


def test_record_accumulates_across_calls():
    tracker = CostTracker()
    tracker.record("openai/gpt-5.6-luna", 500_000, 0)
    tracker.record("openai/gpt-5.6-luna", 500_000, 0)
...
def test_record_response_reads_responses_api_usage_shape():
    ...
    tracker.record_response("openai/gpt-5.6-terra", response)
    assert tracker.input_tokens == 100
    assert tracker.output_tokens == 50


def test_record_response_reads_chat_completions_usage_shape():
    tracker = CostTracker()
...
def test_record_response_reads_embeddings_usage_shape():
    ...
    tracker.record_response("openai/text-embedding-3-small", response)
...
def test_record_response_tolerates_missing_usage():
    tracker = CostTracker()
    tracker.record_response("openai/gpt-5.6-luna", SimpleNamespace(usage=None))
    assert tracker.calls == 0


def test_summary_rounds_and_groups_by_model():
    tracker = CostTracker()
    tracker.record("openai/gpt-5.6-luna", 1_000_000, 0)
    tracker.record("openai/gpt-5.6-terra", 1_000_000, 0)
    summary = tracker.summary()
    assert summary["calls"] == 2
    assert summary["usd"] == round(0.20 + 2.00, 4)
    assert set(tracker.by_model.keys()) == {"openai/gpt-5.6-luna", "openai/gpt-5.6-terra"}
